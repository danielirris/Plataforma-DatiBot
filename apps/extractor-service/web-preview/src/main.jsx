...
function EmojiPop({ emoji, idx }) {
  ...
  return <div style={{ fontFamily: emojiFamily, position: 'absolute', left: p[0], top: p[1], fontSize: size, opacity: o, transform: `translateY(${float}px) rotate(${rot}deg) scale(${0.4 + 0.6 * Math.min(1, enter)})`, filter: 'drop-shadow(0 8px 14px rgba(0,0,0,0.4))' }}>{emoji}</div>;
}
...
function GuidePiP({ file, bg, accent, src }) {
  const { fps, width, durationInFrames } = useVideoConfig();
  const f = useCurrentFrame();
  const enter = spring({ frame: f, fps, config: { damping: 16, mass: 0.6 } });
  const out = interpolate(f, [durationInFrames - 8, durationInFrames], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' });
  const o = Math.min(1, enter) * out;
  const blur = Math.round(26 * Math.min(1, enter));
  const float = Math.sin(f / fps * 2.2) * 6;
  const w = Math.round(width * 0.46);
  return (
    <AbsoluteFill style={{ opacity: o }}>
      {bg ? <AbsoluteFill style={{ overflow: 'hidden' }}><Video src={src(bg)} muted loop style={{ width: '100%', height: '100%', objectFit: 'cover', filter: `blur(${blur}px)`, transform: 'scale(1.18)' }} /></AbsoluteFill> : null}
      <AbsoluteFill style={{ backgroundColor: 'rgba(0,0,0,0.45)' }} />
      <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ position: 'relative', transform: `translateY(${float + (1 - enter) * 40}px) scale(${0.9 + 0.1 * Math.min(1, enter)})` }}>
          <div style={{ position: 'relative', width: w, borderRadius: 24, overflow: 'hidden', border: `6px solid ${accent}`, boxShadow: '0 24px 60px rgba(0,0,0,0.65)', background: '#000' }}>
...
function Ad({ v, cta, musica, sfx, assetBase }) {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: 'black', overflow: 'hidden' }}>
      ...
      {sfx && sfx.impact ? (
        <Sequence from={0} durationInFrames={Math.round(0.6 * fps)}><Audio src={src(sfx.impact)} volume={0.95} /></Sequence>
      ) : null}
      {/* Tick de notificacion en cada pildora (palabra destacada), mas presente. */}
      {sfx && sfx.tick ? (plan.pills || []).map((p, i) => (
        <Sequence key={`tk${i}`} from={Math.round((p.start || 0) * fps)} durationInFrames={Math.round(0.12 * fps)}><Audio src={src(sfx.tick)} volume={0.55} /></Sequence>
      )) : null}
      ...
      {guias.map((g, i) => (
        <Sequence key={`guia${i}`} from={g.f} durationInFrames={g.df}><GuidePiP file={g.file} bg={v.video} accent={pick(i + 3)} src={src} /></Sequence>
      ))}
      {cards.map((c, i) => (

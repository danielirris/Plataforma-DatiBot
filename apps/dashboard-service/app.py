...
# ── Guard de configuración (añadido para el monorepo) ─────────────────────────
# Sin credenciales el dashboard no puede cargar datos. En vez de reventar con un
# traceback de Supabase, mostramos un aviso que apunta a la sección Configuración
# del shell (que genera este .env). El original no tenía este guard.
_has_fb = bool(os.getenv("FB_TOKEN_1") or os.getenv("FB_ACCESS_TOKEN"))
_missing = [
    name
    for name, ok in (
        ("Supabase (SUPABASE_URL / SUPABASE_KEY)", bool(os.getenv("SUPABASE_URL")) and bool(os.getenv("SUPABASE_KEY"))),
        ("Facebook (FB_TOKEN_1 o FB_ACCESS_TOKEN)", _has_fb),
    )
    if not ok
]
if _missing:
    st.title("📊 Dashboard ads")
    st.warning(
        "Faltan credenciales para cargar datos. Ve a **⚙️ Configuración** en el "
        "menú lateral, completa las keys de **Dashboard ads** y guarda; luego "
        "reinicia este servicio.\n\nPendiente: " + ", ".join(_missing)
    )
    st.stop()
# ──────────────────────────────────────────────────────────────────────────────
